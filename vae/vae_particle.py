...
class VAEparticle(vbase.VAE):
	...
	def build_encoder(self, mean, stdev):
		...
		# Dense * 3
		x = tf.keras.layers.Dense(int(self.params.z_sz*17), activation='relu', kernel_regularizer=self.regularizer)(x)  # reduce convolution output
		x = tf.keras.layers.Dense(int(self.params.z_sz*4), activation='relu', kernel_regularizer=self.regularizer)(x)  # reduce again
	...
	def build_decoder(self, mean, stdev):
		...
		# Dense * 3
		x = tf.keras.layers.Dense(int(self.params.z_sz*4), activation='relu', kernel_regularizer=self.regularizer)(latent_inputs)  # inflate to input-shape/200
		x = tf.keras.layers.Dense(int(self.params.z_sz*17), activation='relu', kernel_regularizer=self.regularizer)(x)  # double size
