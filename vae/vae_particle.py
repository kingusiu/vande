...
class Conv1DTranspose(tf.keras.layers.Layer):
    ...
    def get_config(self):
        ...
        config.update({'kernel_sz': self.kernel_sz, 'filters': self.filters, 'activation': self.activation, 'ExpandChannel': self.ExpandChannel, 'ConvTranspose': self.ConvTranspose, 'SqueezeChannel': self.SqueezeChannel})
        return config


class VAEparticle():

	def __init__(self, input_shape=(100,3), z_sz=10, filter_n=6, kernel_sz=3, loss=losses.mse_loss, beta=0.01, regularizer=None):
	...
	def build_encoder(self, inputs, mean, var):
        ...
        # 2D Conv
        x = tf.keras.layers.Conv2D(filters=self.filter_n, kernel_size=self.kernel_sz, activation='relu', kernel_regularizer=self.regularizer)(x)
        # Squeeze
        ...
        # 1D Conv * 2
        self.filter_n += 4
        x = tf.keras.layers.Conv1D(filters=self.filter_n, kernel_size=self.kernel_sz, activation='relu', kernel_regularizer=self.regularizer)(x) # [ B x 96 x 10 ]
        self.filter_n += 4
        x = tf.keras.layers.Conv1D(filters=self.filter_n, kernel_size=self.kernel_sz, activation='relu', kernel_regularizer=self.regularizer)(x) # [ B x 94 x 14 ]
        # Pool
        ...
        self.z_mean = tf.keras.layers.Dense(self.z_sz, name='z_mean', kernel_regularizer=self.regularizer)(x)
        self.z_log_var = tf.keras.layers.Dense(self.z_sz, name='z_log_var', kernel_regularizer=self.regularizer)(x)
	...
	def build_decoder(self):
		latent_inputs = tf.keras.layers.Input(shape=(self.z_sz,), name='z_sampling')
        # Dense * 3
        ...
        # 1D Conv Transpose * 2
        self.filter_n -= 4
        x = Conv1DTranspose(filters=self.filter_n, kernel_size=self.kernel_sz, activation='relu')(x) # [ B x 94 x 16 ] -> [ B x 96 x 8 ]
        self.filter_n -= 4
        x = Conv1DTranspose(filters=self.filter_n, kernel_size=self.kernel_sz, activation='relu')(x) # [ B x 96 x 8 ] -> [ B x 98 x 4 ]
        # Expand
        x = tf.keras.layers.Lambda(lambda x: tf.expand_dims(x,axis=2))(x) #  [ B x 98 x 1 x 4 ]
        # 2D Conv Transpose
        x = tf.keras.layers.Conv2DTranspose(filters=1, kernel_size=self.kernel_sz, activation=tf.keras.activations.elu, kernel_regularizer=self.regularizer, name='conv_2d_transpose')(x)
