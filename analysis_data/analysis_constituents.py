...
from analysis_data.analysis import Analysis
...
class AnalysisConstituents(Analysis):
    ...
    def __init__(self, data_name, do=['eta','phi','pt'], fig_dir='fig'):
        ...
        self.d_eta_idx, self.d_phi_idx, self.pt_idx = 0, 1, 2
    ...
    def analyze(self,data):
        ...
        for jet in (0,1):
            for idx, name in enumerate(['eta', 'phi', 'pt']):
                dat = data[jet][:,:,idx]
                up.plot_hist_on_axis(axs[idx,jet],dat.flatten(),xlabel=name+' jet'+str(jet+1))

        plt.suptitle(r'distribution particles $\eta, \phi, pt$ ' + self.data_name)
        plt.tight_layout(rect=(0, 0, 1, 0.95))

        if self.fig_dir:
            fig.savefig(os.path.join( self.fig_dir, 'hist_eta_phi_pt_' + self.fig_name + '.png' ) )
        plt.close(fig)
